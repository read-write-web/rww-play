...
class RWWActorSubdomainsTest extends WordSpec with Matchers {
  ...
  s"base=$stampleBase tests with subdomains on stample" in {
    ...
    local(new URI("https://james.stample.io/.acl"),stampleBase) should be (Some(Switch(Some("james"),"")))
    local(new URI("https://james.stample.io/2013/.acl"),stampleBase) should be (Some(Switch(Some("james"),"2013")))
    local(new URI("https://slim.stample.io/"),stampleBase) should be (Some(Switch(Some("slim"),"")))
  }
  ...
  s"base=$stampleBase2 tests with subdomains on stample inversing ports" in {
    ...
    local(new URI("https://james.stample.io:443/.acl"),stampleBase2) should be (Some(Switch(Some("james"),"")))
    local(new URI("https://james.stample.io:443/2013/.acl"),stampleBase2) should be (Some(Switch(Some("james"),"2013")))
    local(new URI("https://slim.stample.io:443/"),stampleBase2) should be (Some(Switch(Some("slim"),"")))
  }
